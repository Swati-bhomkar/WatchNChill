...
const DateSelect = ({ dateTime, id }) => {
    ...
    const [selected, setSelected] = useState(null) //state to hold selected date
    const onBookHandler = () => { //handle book now button click
        if (!selected) {
                    ...
                    <div className='flex items-center gap-6 text-sm mt-5'>
                        ...
                        <span className='grid grid-cols-3 md:flex flex-wrap md:max-w-lg gap-4'>
                            {Object.keys(dateTime).map((date) => (
                                ...
                                justify-center h-14 w-14 aspect-square rounded cursor-pointer ${selected === date ?     //highlight selected date
                                        "bg-primary text-white" : "border border-primary/70"}`}>
                                    <span>{new Date(date).getDate()}</span>
                                    <span>{new Date(date).toLocaleDateString("en-US", { month: "short" })}</span> {/*format month to short form e.g Jan, Feb*/}
